...
export default function DashboardLayout({ children }: DashboardLayoutProps) {
  ...
  const filteredNavigation = navigationItems.filter(item => 
    user && item.roles.includes(user.type)
  );
  ...
  return (
    <div className={`min-h-screen bg-background ${isArabic ? 'rtl' : 'ltr'}`} lang={isArabic ? 'ar' : 'en'}>
      {/* Sidebar */}
      <aside className={`fixed inset-y-0 z-50 flex w-72 flex-col transition-transform duration-300 ${
          ...
          <div className="flex h-16 shrink-0 items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <div className="flex flex-col">
                ...
                <span className="text-xs text-sidebar-foreground/60">نظام إدارة متكامل</span>
              </div>
            </div>
            <Button
              variant="ghost"
